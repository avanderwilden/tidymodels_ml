...
smote_wf <- workflow(smote_rec, xgb_spec)
```


```{r}
set.seed(917)
xgb_grid <- grid_latin_hypercube(xgb_params, size = 25)
```
...
```{r}
show_best(smote_res, metric = 'mn_log_loss')
```
...
```{r}
smote_last <- smote_wf %>% 
  finalize_workflow(select_best(smote_res, 'mn_log_loss')) %>% 
  last_fit(bb_split)
...
xgb_last %>% 
  collect_predictions() %>% 
  conf_mat(truth = is_home_run, estimate = .pred_class)
```
